def ct(i,dur,g,tp):#i means starting time
    flag = 0
    for x in tp:
        for y in g[x]:
...
l=[]
p=[]
with open("updated.csv", "w") as meeting:
    meet = csv.writer(meeting)
    meet.writerow(["Employee IDs", "meeting time"])
...
while t:
    ...
    with open("updated.csv", "a") as meeting:
            meet= csv.writer(meeting)
            meet.writerow([*tp,"{}--{}".format(sm, sm+dur)])
